...
<br/>
**<center>学号：251___</center>**
<br/>
**<center>日期：2025年11月8日</center>**
<br/><br/><br/><br/><br/><br/><br/>
<br/><br/>
## 摘要
&emsp;&emsp;本文从零实现了完整的Transformer模型架构，包括多头自注意力机制、位置编码、前馈神经网络、残差连接和层归一化等核心组件。在IWSLT2017英德翻译数据集上进行了模型训练和评估，并设计了系统的消融实验来分析各组件的重要性。实验结果表明，位置编码对模型性能影响最大，其次是注意力头数和模型层数。

&emsp;&emsp;项目已完整开源，代码仓库地址：
<https://github.com/xainyu01/Transformer-Implementation>包含完整的src/源代码/requirements.txt依赖文件、README.md详细说明文档以及scripts/运行脚本，满足课程开源要求。由于预训练模型文件(checkpoint/best_model.pth)体积较大，为避免GitHub仓库臃肿，该文件未直接上传至代码仓库，可通过提供的训练脚本重新训练生成，或从以下地址下载：通过网盘分享的文件：best_model.pth
<https://pan.baidu.com/s/1CUkdbsna56PJdhjzrKiYpQ?pwd=ibdn >S提取码: ibdn 

&emsp;&emsp;本文提供了完整的代码实现、训练曲线和消融分析，为深入理解Transformer工作机制提供了详实的实践依据和可复现的实验基础。
...
<p align="center"><img src="https://latex.codecogs.com/gif.latex?\text{Attention}(Q,K,V)%20=%20\text{softmax}\left(\frac{QK^T}{\sqrt{d_k}}\right)V"/></p>  
其中：
- <img src="https://latex.codecogs.com/gif.latex?Q%20\in%20\mathbb{R}^{n%20\times%20d_k}"/>：查询矩阵
- <img src="https://latex.codecogs.com/gif.latex?K%20\in%20\mathbb{R}^{m%20\times%20d_k}"/>：键矩阵  
- <img src="https://latex.codecogs.com/gif.latex?V%20\in%20\mathbb{R}^{m%20\times%20d_v}"/>：值矩阵
- <img src="https://latex.codecogs.com/gif.latex?d_k"/>：键向量的维度
- <img src="https://latex.codecogs.com/gif.latex?\sqrt{d_k}"/>：缩放因子，防止softmax梯度消失
  
### 3.5 Multi-Head Attention

多头注意力将输入投影到多个子空间，分别计算注意力后拼接：

<p align="center"><img src="https://latex.codecogs.com/gif.latex?\text{MultiHead}(Q,K,V)%20=%20\text{Concat}(head_1,%20\ldots,%20head_h)W^O"/></p>  
<p align="center"><img src="https://latex.codecogs.com/gif.latex?\text{其中%20}%20head_i%20=%20\text{Attention}(QW_i^Q,%20KW_i^K,%20VW_i^V)"/></p>  
多头机制的优势：
- 捕捉不同类型的依赖关系
- 增强模型的表示能力
- 提高训练稳定性
...
<p align="center"><img src="https://latex.codecogs.com/gif.latex?\text{FFN}(x)%20=%20\max(0,%20xW_1%20+%20b_1)W_2%20+%20b_2"/></p>  
其中<img src="https://latex.codecogs.com/gif.latex?W_1%20\in%20\mathbb{R}^{d_{model}%20\times%20d_{ff}}"/>，<img src="https://latex.codecogs.com/gif.latex?W_2%20\in%20\mathbb{R}^{d_{ff}%20\times%20d_{model}}"/>。

### 3.7 残差连接与层归一化

残差连接缓解梯度消失问题：

<p align="center"><img src="https://latex.codecogs.com/gif.latex?\text{LayerNorm}(x%20+%20\text{Sublayer}(x))"/></p>  
层归一化对每个样本的所有特征进行归一化：

<p align="center"><img src="https://latex.codecogs.com/gif.latex?\text{LayerNorm}(x)%20=%20\gamma%20\frac{x%20-%20\mu}{\sigma}%20+%20\beta"/></p>  
### 3.8 位置编码

使用正弦和余弦函数的位置编码：

<p align="center"><img src="https://latex.codecogs.com/gif.latex?PE_{(pos,2i)}%20=%20\sin\left(\frac{pos}{10000^{2i/d_{model}}}\right)"/></p>  
<p align="center"><img src="https://latex.codecogs.com/gif.latex?PE_{(pos,2i+1)}%20=%20\cos\left(\frac{pos}{10000^{2i/d_{model}}}\right)"/></p>  
## 4 实现细节
...
### 5.3 评估指标
- **交叉熵损失**：训练和验证损失
- **困惑度**：<img src="https://latex.codecogs.com/gif.latex?\text{PPL}%20=%20\exp(\text{loss})"/>
- **翻译质量**：人工评估生成结果
